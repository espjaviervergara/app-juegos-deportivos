...
class PartidoController
{
    ...
    public function store(Request $req): void
    {
        ...
        $a=(int)($req->input('equipoAId')??0); $b=(int)($req->input('equipoBId')??0); $fh=$req->input('fechaHora'); $grupoId=$req->input('grupoId')? (int)$req->input('grupoId'):null;
        ...
        if($grupoId){
            $g=$pdo->prepare("SELECT torneo_id FROM grupos WHERE id=?"); $g->execute([$grupoId]); $tg=$g->fetchColumn(); if(!$tg||$tg!=$torneo) Response::error('VALIDATION_ERROR','Grupo no pertenece a torneo',422);
            foreach([$a,$b] as $eid){ $chk=$pdo->prepare("SELECT 1 FROM grupo_equipo WHERE grupo_id=? AND equipo_id=?"); $chk->execute([$grupoId,$eid]); if(!$chk->fetch()) Response::error('VALIDATION_ERROR',"Equipo $eid no está en grupo $grupoId",422); }
        }
        $this->checkOverlap($a,$fh); $this->checkOverlap($b,$fh);
        $pdo->prepare("INSERT INTO partidos (jornada_id, equipoA_id, equipoB_id, fechaHora, grupo_id) VALUES (?,?,?,?,?)")->execute([$jid,$a,$b,$fh,$grupoId]);
    ...
    public function update(Request $req): void
    {
        ...
        $jid=$req->input('jornadaId')??$p['jornada_id']; $a=$req->input('equipoAId')??$p['equipoA_id']; $b=$req->input('equipoBId')??$p['equipoB_id']; $fh=$req->input('fechaHora')??$p['fechaHora']; $grupoId=$req->input('grupoId')!==null ? (int)$req->input('grupoId') : $p['grupo_id'];
        if($a==$b) Response::error('VALIDATION_ERROR','Teams must be different',422);
        // validar pertenencia
        ...
        $pdo->prepare("UPDATE partidos SET jornada_id=?, equipoA_id=?, equipoB_id=?, fechaHora=?, grupo_id=? WHERE id=?")->execute([$jid,$a,$b,$fh,$grupoId,$id]);
