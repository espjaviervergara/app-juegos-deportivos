  ...
  return (
    <div>
      ...
      <table className="table table-striped table-bordered">
        ...
        <tbody>{rows.map((r,i)=>{
          const pos=(page-1)*limit+i+1; const medalla=pos===1?'🥇':pos===2?'🥈':pos===3?'🥉':pos
          return <tr key={r.equipo_id} className={pos===1?'table-warning':''}><td>{medalla}</td><td>{r.equipo}</td><td>{r.PJ}</td><td>{r.PG}</td><td>{r.PE}</td><td>{r.PP}</td><td>{r.GF}</td><td>{r.GC}</td><td>{r.GA}</td><td className="tw-font-bold">{r.puntos}</td></tr>
        })}</tbody>
      </table>
