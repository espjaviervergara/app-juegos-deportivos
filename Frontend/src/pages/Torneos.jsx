    ...
    <div>
      ...
      {data.length===0 ? <div className="alert alert-info">No hay torneos</div> : <div className="row">{data.map(t=>(
        <div key={t.id} className="col-md-4 mb-3"><div className="card"><div className="card-body">
          ...
          <div className="mt-2"><Link className="btn btn-sm btn-primary me-1" to={`/torneos/${t.id}`}>Ver</Link><Link className="btn btn-sm btn-outline-primary" to={`/torneos/${t.id}/calendario`}>Calendario</Link></div>
        </div></div></div>
      ))}</div>}
